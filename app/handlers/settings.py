...
async def manage_wallets(callback: CallbackQuery):
    ...
    # Existing wallets - option to view/regenerate
    for w in wallets:
        config = NETWORKS.get(w.network)
        if config:
            buttons.append([InlineKeyboardButton(
                text=f"{config.icon} {config.symbol} ({w.address[:8]}...)",
                callback_data=f"wallet_detail:{w.network}"
            )])
    ...
    for network, config in NETWORKS.items():
        if network not in existing_networks:
            missing_count += 1
            if missing_count <= 5:  # Show max 5 missing
                buttons.append([InlineKeyboardButton(
                    text=f"➕ Create {config.icon} {config.symbol}",
                    callback_data=f"create_wallet:{network}"
                )])
    
    if missing_count > 5:
        buttons.append([InlineKeyboardButton(
            text=f"➕ Create More ({missing_count - 5} available)",
            callback_data="show_all_networks"
        )])
    
    buttons.append([InlineKeyboardButton(text=get_text("btn_back", lang), callback_data="settings")])
    
    await safe_edit_text(
        callback.message,
        "💼 <b>Manage Wallets</b>\n\n"
        f"You have {len(wallets)} wallet(s)\n\n"
        "Tap a wallet to view details or create new ones:",
        InlineKeyboardMarkup(inline_keyboard=buttons)
    )
    await callback.answer()


@router.callback_query(F.data == "show_all_networks")
async def show_all_networks(callback: CallbackQuery):
    """Show all available networks to create"""
    async with db_manager.session() as session:
        user_repo = UserRepository()
        user = await user_repo.get_by_telegram_id(session, callback.from_user.id)
        lang = user.language_code or "en"
    ...
    buttons.append([InlineKeyboardButton(text=get_text("btn_back", lang), callback_data="manage_wallets")])
...
async def wallet_detail(callback: CallbackQuery):
    ...
    async with db_manager.session() as session:
        ...
        user = await user_repo.get_by_telegram_id(session, callback.from_user.id)
        lang = user.language_code or "en"
        wallet = await WalletRepository().get_user_wallet_by_network(session, user.id, network)
    ...
    await safe_edit_text(
        ...
        f"<b>Balance:</b> {total_balance:.6f} {config.symbol}",
        InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📋 Copy Address", callback_data=f"copy_addr:{network}")],
            [InlineKeyboardButton(text="🔄 Regenerate", callback_data=f"regen_wallet:{network}")],
            [InlineKeyboardButton(text=get_text("btn_back", lang), callback_data="manage_wallets")]
        ])
    )
    await callback.answer()
...
async def copy_address(callback: CallbackQuery):
    """Copy wallet address notification"""
    network = callback.data.split(":")[1]
    ...
    async with db_manager.session() as session:
        ...
        if wallet:
            await callback.answer(f"Address: {wallet.address}", show_alert=True)
        else:
            await callback.answer("Wallet not found", show_alert=True)
...
async def create_new_wallet(callback: CallbackQuery):
    ...
    await safe_edit_text(
        callback.message,
        f"⏳ Creating {config.symbol} wallet...",
    )
    ...
    try:
        async with db_manager.session() as session:
            ...
            existing = await WalletRepository().get_user_wallet_by_network(session, user.id, network)
            if existing:
                await safe_edit_text(
                    callback.message,
        ...
        await safe_edit_text(
            ...
            f"You can now receive {config.symbol} on this address!",
            InlineKeyboardMarkup(inline_keyboard=[
        ...
        logger.error("create_wallet error", error=str(e))
        await safe_edit_text(
            callback.message,
            f"❌ Failed to create wallet\n\n"
            f"Error: {str(e)}",
            InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_text("btn_back", "en"), callback_data="manage_wallets")]
            ])
        )
...
async def confirm_regenerate(callback: CallbackQuery):
    ...
    await safe_edit_text(
        ...
        f"❌ Any funds on the old address will be INACCESSIBLE!\n\n"
        f"<b>Are you absolutely sure?</b>",
        InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Yes, Regenerate", callback_data=f"regen_confirm:{network}")],
            [InlineKeyboardButton(text="❌ Cancel", callback_data=f"wallet_detail:{network}")]
        ])
    )
    await callback.answer()
...
async def do_regenerate(callback: CallbackQuery):
    ...
    await safe_edit_text(
        callback.message,
        f"⏳ Regenerating {config.symbol} wallet...",
    )
    ...
    try:
        async with db_manager.session() as session:
            ...
            # Delete old wallet and balances
            old_wallet = await WalletRepository().get_user_wallet_by_network(session, user.id, network)
            
            if old_wallet:
                # Delete balances
                from sqlalchemy import delete
                await session.execute(
                    delete(WalletBalance).where(WalletBalance.wallet_id == old_wallet.id)
                )
                # Delete wallet
        ...
        await safe_edit_text(
            ...
            f"{config.icon} <b>{config.name}</b>\n\n"
            f"<b>New Address:</b>\n<code>{wallet_data.address}</code>\n\n"
            f"⚠️ Old address is no longer accessible!",
            InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="💼 Manage Wallets", callback_data="manage_wallets")],
                [InlineKeyboardButton(text="🏠 Main Menu", callback_data="main_menu")]
            ])
        ...
        logger.error("regenerate_wallet error", error=str(e))
        await safe_edit_text(
            callback.message,
            f"❌ Failed to regenerate wallet\n\n"
            f"Error: {str(e)}",
            InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=get_text("btn_back", "en"), callback_data="manage_wallets")]
            ])
